...
console.log('id',id)
        let index=emailList.findIndex(el=>el.id===id)
        console.log('index',index)
        let list=[...emailList]
        list.splice(index,1)
        setEmailList(list)
    }
    const editItem=(id:string,email:string)=>{
        let index=emailList.findIndex(el=>el.id===id)
        if(email){
    ...
    return (
        ...
        >
            ...
            <Typography.H5 m={3} sx={{ lineHeight: 1 }}>Invite User</Typography.H5>
            ...
            <Flex mx={[0, 3]}>
                <Button  sx={{width: "100%"}}>Send Invitation</Button>
            </Flex>
...
}> = ({ id,email,editItem,removeFromEmailList }) => {
    return (
        <Card
            p={3}
            mx={[0, 3]}
...
const PlaceholderCard: FC<{
    ...
    return (
        ...
        >
            <Button variant="outline" sx={{height: "100%"}}>
                    <Icon as={PlusIcon} m={0} onClick={()=>addToEmailList([...emailList,{'id':new Date().getTime().toString(),'email':''}])} />
            </Button>
                <Flex sx={{ flexDirection: "column", alignItems: "start" }}>
                    <Typography.Body ml={3} sx={{ lineHeight: 1.6 }}>Add More Email</Typography.Body>
                </Flex>
        </Card>
    )
}
