// TODO: remove when @types/react-query v1 is release
// Temporary definition while waiting for v1 types to be released
declare module "react-query" {
   ...
   */
  export type FetchMoreVariable<T extends QueryKey> = T extends [
    string,
    infer U,
    ...any[]
  ]
  ...
  export interface InfiniteQueryOptions<TResult, FetchMoreVariable>
    extends QueryOptions<TResult> {
    getFetchMore?:
      | ((lastGroup: TResult, allGroups: TResult[]) => FetchMoreVariable)
      | boolean
  }
  ...
  export interface QueryBaseState {
    isFetching: boolean
    failureCount: number
    refetch: (options?: { force?: boolean; thrownOnError?: boolean }) => void
  }
  ...
  export interface InfiniteQueryLoadingState<TVariable>
    extends InfiniteQueryBaseState<TVariable> {
    status: "loading"
    data: []
    error: null
  }

  export interface InfiniteQueryErrorState<TVariable>
    extends InfiniteQueryBaseState<TVariable> {
    status: "error"
    data: []
    error: Error
  ...
  export interface InfiniteQuerySuccessState<TResult, TVariable>
    extends InfiniteQueryBaseState<TVariable> {
    status: "success"
    data: TResult[]
    error: null
  ...
  export namespace queryCache {
    ...
    function subscribe(
      callback: (queryCache: unknown) => void
